...
exports.create = async (req, res) => {
  ...
  if (jsonRpcUri === "default") {
    console.log(
      `Using "default" provider url (from tokens) = ${paymentToken.providerUrl}`
    );
  ...
  try {
    ...
    errorResponse(
      ...
      err,
      500,
      "Unable to get price from payment processor."
    );
    return;
  }
  ...
  const uploadFeePlusBuffer = priceWei.add(
    priceWei.div(process.env.BUNDLR_PRICE_BUFFER ?? 10)
  );
  ...
  try {
    ...
    console.error(
      `Error occurred while estimating gas. Using default gas estimate. ${err?.name}: ${err?.message}`
    );
  }

  // Assume all payment chains support EIP-1559 transactions.
  const gasFeeEstimate = gasEstimate.mul(
    feeData.maxFeePerGas.add(feeData.maxPriorityFeePerGas)
  );
  console.log(`gasFeeEstimate = ${gasFeeEstimate}`);

  const gasFeePlusBuffer = gasFeeEstimate.add(
    gasFeeEstimate.div(process.env.GAS_PRICE_BUFFER ?? 10)
  );
  ...
  try {
    ...
    errorResponse(
      ...
      err,
      500,
      `Error occurred while getting server fee token balance.`
    );
    return;
  }
  ...
  console.log("uploadFeePlusBuffer", uploadFeePlusBuffer);
  const tokenAmount = uploadFeePlusBuffer
    .mul(buffer)
    .add(gasFeePlusBuffer)
    .toString();
